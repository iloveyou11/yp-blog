...
shpManager.prototype.publishdataStore = function(datastore, cb) {
    ...
    shpManager.POST(`${FolderPath["dataStoreFolder"]}\\${datastore}.xml`, url, cb, datastore);
};
...
shpManager.prototype.updatedataStore = function(datastore, cb) {
    ...
    shpManager.PUT(`${FolderPath["dataStoreFolder"]}\\${datastore}.xml`, url, cb, 1);
};
...
shpManager.prototype.publishLayer = function(layer, cb) {
    ...
    shpManager.POST(`${FolderPath["LayerFolder"]}\\${layer}.xml`, url, cb, "");
};
...
shpManager.prototype.updateLayer = function(layer, cb) {
    ...
    shpManager.PUT(`${FolderPath["LayerFolder"]}\\${layer}.xml`, url, cb, 1);
};
...
shpManager.prototype.CreatedataStoreXml = function(storename, type) {
    this.storename = storename;
    this.shpfile = `${FolderPath["dataStoreFolder"]}\\${storename}.xml`;
    return new Promise((resolve, reject) => {
        let parser = new xml2js.Parser(); //用于把xml对象解析为json
        let builder = new xml2js.Builder(); //用于把json对象解析为xml
        fs.readFile(`${FolderPath["xmlFolder"]}\\datastore.xml`, (err, data) => {
            parser.parseString(data, (err, result) => {
                for (key in result["dataStore"]) {
                    switch (key) {
                        ...
                        case "connectionParameters":
                            let folder = "uploadBuildingFolder";
                            if (type === "land") {
                                folder = "uploadLandFolder";
                            }
                            let filepath = `${FolderPath[folder]}\\${storename}\\${storename}.shp`;
                            result["dataStore"][key][0]["entry"][8]["_"] = `file://${filepath}`;
                            break;
                        case "featureTypes":
                ...
                fs.writeFile(
                    `${FolderPath["dataStoreFolder"]}\\${storename}.xml`,
                    outxml.toString(),
                    (err1) => {
                        if (err1) {
                            return reject(err1);
...
shpManager.prototype.CreateLayerXml = function(layer, layername) {
    this.layer = layername;
    this.layerxml = `${FolderPath["LayerFolder"]}\\${layername}.xml`;
    return new Promise((resolve, reject) => {
        let parser = new xml2js.Parser(); //用于把xml对象解析为json
        let builder = new xml2js.Builder(); //用于把json对象解析为xml
        fs.readFile(`${FolderPath["xmlFolder"]}\\layer.xml`, (err, data) => {
            if (!err) {
                parser.parseString(data, (err, result) => {
                    if (!err) {
                        for (key in result) {
                            ...
                            for (key in val) {
                                ...
                                switch (key) {
                                    ...
                                    case "latLonBoundingBox":
                                        ...
                                        for (key in temp) {
                                            if (key != "crs")
                                                temp[key] = [String(bounds[name[index++]] + change[index % 2])];
                                        }
                        ...
                        try {
                            fs.writeFileSync(
                                `${FolderPath["LayerFolder"]}\\${layername}.xml`,
                                outxml.toString(),
                            );
                            return resolve();
...
var shpManager = new shpManager({
    ...
    storename: "D11",
    layer: "D11",
    shpfile: `${FolderPath["dataStoreFolder"]}\\D11.xml`,
    layerxml: `${FolderPath["LayerFolder"]}\\D11.xml`,
    stylename: "area",
});

module.exports = {
    publish,
    CreateLayerXml: shpManager.CreateLayerXml,
    CreatedataStoreXml: shpManager.CreatedataStoreXml,
}
```
